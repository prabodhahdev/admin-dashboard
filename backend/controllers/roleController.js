...
//new code based on level and superadmin protection
...
export const createRole = async (req, res) => {
  try {
    ...
    if (normalizedRoleName === "superadmin") {
      ...
      if (superAdminExists) {
        return res.status(400).json({ error: "SuperAdmin role already exists" });
      }
    }
    ...
    if (normalizedRoleName === "superadmin") {
      ...
      if (level === undefined || level < 1) {
        return res.status(400).json({ error: "Level is required and must be >= 1" });
      }
    }
    ...
    if (existingRole) {
      return res.status(400).json({ error: "Role name or level already exists" });
    }
...
export const updateRole = async (req, res) => {
  try {
    ...
    if (role.roleName === "superadmin") {
      if ((roleName && roleName.toLowerCase() !== "superadmin") || level !== undefined) {
        return res.status(400).json({ error: "Cannot rename or change level of SuperAdmin" });
      }
    }
    ...
    if (level !== undefined && level !== role.level) {
      ...
      if (existingLevel && existingLevel._id.toString() !== role._id.toString()) {
